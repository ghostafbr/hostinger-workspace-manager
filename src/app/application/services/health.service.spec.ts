...
describe('HealthService', () => {
  ...
  describe('getAllHealthMetrics', () => {
    it('should calculate metrics for workspaces', async () => {
      ...
      const syncRunsSnap = {
        docs: [
          { id: 'r1', data: () => ({ status: SyncRunStatus.SUCCESS, duration: 1000, startedAt: { toDate: () => new Date() } }) },
          { id: 'r2', data: () => ({ status: SyncRunStatus.FAILED, errorMessage: 'Timeout', startedAt: { toDate: () => new Date() } }) },
        ]
      };
