...
<div class="dashboard-container" @fadeIn role="main">
  ...
  <p-toolbar class="dashboard-toolbar">
          ...
          <div class="welcome-inline">
            ...
            @if (hasIssues()) {
              <span class="status-text warning-text">
                ...
                {{ stats()!.domainsExpiring7Days + stats()!.subscriptionsExpiring7Days }} vencimientos
              </span>
            } @else {
    ...
    <div class="p-toolbar-group-end">
      <p-button label="Ver Workspaces" icon="pi pi-briefcase" severity="success" size="small" (onClick)="navigateToWorkspaces()" class="mr-2" />
      <p-splitButton label="Actualizar" icon="pi pi-refresh" [model]="toolbarActions()" severity="secondary" [outlined]="true" size="small" (onClick)="loadDashboard()"></p-splitButton>
    </div>
  </p-toolbar>
    ...
    @if (stats() && !isLoading()) {
      <div class="stats-section-container">
        ...
        <div class="compact-stats-grid">
           <app-expiration-card title="Dominios" icon="pi pi-globe" [totalCount]="stats()!.totalDomains" [count7Days]="stats()!.domainsExpiring7Days" [count15Days]="stats()!.domainsExpiring15Days" [count30Days]="stats()!.domainsExpiring30Days" [count60Days]="stats()!.domainsExpiring60Days" class="glass-card full-height" />
           <app-expiration-card title="Suscripciones" icon="pi pi-ticket" [totalCount]="stats()!.totalSubscriptions" [count7Days]="stats()!.subscriptionsExpiring7Days" [count15Days]="stats()!.subscriptionsExpiring15Days" [count30Days]="stats()!.subscriptionsExpiring30Days" [count60Days]="stats()!.subscriptionsExpiring60Days" class="glass-card full-height" />
      ...
      <div class="mb-4">
        <app-advanced-search (searchApplied)="onSearchApplied($event)" (searchCleared)="onSearchCleared()" />
      </div>
  ...
  styles: [`
    ...
    .dashboard-toolbar { margin-bottom: 2rem; animation: slideDown 0.3s ease-out; }
    .dashboard-title { display: flex; align-items: center; gap: 0.75rem; font-weight: 700; font-size: 1.5rem; }
    .dashboard-content { margin: 0 auto; }
    .stats-section-container { margin-bottom: 2rem; }
    .compact-stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; }
    .glass-card { background: white; border-radius: 20px; padding: 1.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.03); }
  `],
})
